...
def upgrade() -> None:
    ...
    op.add_column("mr_nas_manager", sa.Column("retention_days", sa.Integer(), nullable=True))
    op.add_column(
        "mr_nas_manager", sa.Column("last_fetch_at", sa.DateTime(timezone=True), nullable=True)
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "manager_id",
            sa.Integer(),
            sa.ForeignKey("mr_nas_manager.id", ondelete="CASCADE"),
            nullable=False,
        ),
        sa.Column(
    ...
    op.create_index(
        "ix_nas_config_version_nas_created", "mr_nas_config_version", ["nas_id", "created_at"]
    )
