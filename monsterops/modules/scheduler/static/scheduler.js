...
class SchedulerPage extends HTMLElement {
  ...
  async _loadJobs() {
    try {
      this._jobs = await api.get('/scheduler/jobs');
    } catch {
      this._jobs = [];
    }
  ...
  _renderJobs(tc) {
    if (!this._jobs.length) {
      tc.innerHTML =
        '<div class="state-empty">No scheduled jobs. Click <strong>+ New Job</strong> to schedule your first report.</div>';
      return;
    }
    tc.innerHTML = `
      <div class="card">
  ...
  async _renderReports(tc) {
    ...
    try {
      ...
      if (!reports.length) {
        tc.innerHTML =
          '<div class="state-empty">No reports generated yet. Jobs run on their configured schedule.</div>';
        return;
      }
      tc.innerHTML = `
        <div class="card">
          ...
          r.status === 'ok'
            ...
            : '<span class="badge badge-err">Error</span>'
                  ...
                  <td style="font-size:0.75rem;color:var(--color-muted);">
                    ${
          r.data
            ? `${r.data.period_start?.slice(0, 10) || ''} – ${
              r.data.period_end?.slice(0, 10) || ''
            }`
            : (r.error_message ? esc(r.error_message.slice(0, 60)) : '—')
        }
                  </td>
                  <td>
                    ${
          r.data
            ? `<button class="btn btn-sm" data-action="view" data-report='${
  ...
  async _deleteJob(id) {
    if (
      !(await confirmDialog('Delete this scheduled job?', { title: 'Delete job', danger: true }))
    ) return;
    try {
  ...
  _openModal(id) {
    ...
    const body = this.shadowRoot.getElementById('modal-body');
    this.shadowRoot.getElementById('modal-title').textContent = j
      ? 'Edit Job'
      : 'New Scheduled Job';
    ...
    body.innerHTML = `
      <div class="field">
        ...
        <input class="input" id="j-name" value="${
      esc(j?.name ?? '')
    }" placeholder="Daily Summary" ${j ? 'readonly' : ''} />
      </div>
      <div class="field">
      ...
      j?.job_type === 'daily_summary' ? 'selected' : ''
    }>Daily Summary</option>
            <option value="weekly_summary"        ${
      j?.job_type === 'weekly_summary' ? 'selected' : ''
    }>Weekly Summary</option>
          </optgroup>
          <optgroup label="Automation">
            <option value="expired_user_cleanup"  ${
      j?.job_type === 'expired_user_cleanup' ? 'selected' : ''
    }>Expired User Cleanup</option>
            <option value="stale_session_sweep"   ${
      j?.job_type === 'stale_session_sweep' ? 'selected' : ''
    }>Stale Session Sweep</option>
            <option value="log_retention"         ${
      j?.job_type === 'log_retention' ? 'selected' : ''
    }>Log Retention (prune old logs)</option>
          </optgroup>
        </select>
      </div>
      <div class="form-row">
        <div class="field">
          ...
          <input class="input" id="j-hour" type="number" min="0" max="23" value="${
      j?.cron_hour ?? 0
    }" />
        </div>
        <div class="field">
          <label>Minute (0–59)</label>
          <input class="input" id="j-minute" type="number" min="0" max="59" value="${
      j?.cron_minute ?? 0
    }" />
        </div>
      </div>
      <div class="field">
        ...
        <select class="select" id="j-weekday">
          ...
          ${
      WEEKDAYS.map((d, i) =>
        `<option value="${i}" ${j?.cron_weekday === i ? 'selected' : ''}>${d}</option>`
      ).join('')
    }
        </select>
      </div>
      <div class="field">
        ...
        <input class="input" id="j-recipients" value="${
      esc((j?.recipients || []).join(', '))
  ...
  async _submitModal() {
    ...
    clearFieldErrors(sr);
    if (!name) {
      setFieldError(sr.getElementById('j-name'), 'Job name is required');
      return;
    }
    try {
      if (this._editingId != null) {
        await api.put(`/scheduler/jobs/${this._editingId}`, {
          cron_hour,
          cron_minute,
          cron_weekday,
          recipients,
      ...
      if (!applyServerErrors(sr, e, (f) => sr.getElementById('j-' + f.replace(/_/g, '-')))) {
        toast(e.message, 'error');
      }
    }
  }
