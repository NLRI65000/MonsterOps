...
def job_fn_for(job_type: str):
    from monsterops.modules.scheduler.jobs import (
        run_daily_summary, run_expired_user_cleanup, run_log_retention,
        run_stale_session_sweep, run_weekly_summary,
    )
...
def schedule_job(job_id: int, job_type: str, cron_hour: int, cron_minute: int,
                 cron_weekday: int | None, job_name: str, recipients: list[str]) -> None:
    sched = get_scheduler()
    jid = _job_id(job_id)
    ...
    logger.info("Scheduled job %s (%s) at %02d:%02d dow=%s", job_name, job_type, cron_hour, cron_minute, dow)
