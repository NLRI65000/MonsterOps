  ...
  if (factory) {
    try {
      ...
      msg.innerHTML = `
        ...
        <pre style="color:var(--color-muted);font-size:0.75rem;white-space:pre-wrap;font-family:monospace;">${String(err)}</pre>
      `;
    ...
    msg.textContent = path === '/' ? 'Select a section from the sidebar.' : `No page yet for ${full} — coming soon.`;
    view.replaceChildren(msg);
  }
}
